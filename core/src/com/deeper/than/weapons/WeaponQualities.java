...
public enum WeaponQualities { //accuracy, damageModifier, critdamage, chance, rechargeSpeed, cost, power
	...
	LIKENEW("Like New",new Range(.50f, .65f), new Range(1,1),new Range(1.5f,2),new Range(.20f,30f),new Range(1,1),new Range(1,1.5f),new Range(0,2)),
	...
	private int getIntValueInRange(int min, int max){
		int range = max - min;
		int value = ran.nextInt(range)+min;
		return value;
	}
